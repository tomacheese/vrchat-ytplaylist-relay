...
export async function getOrDownload(
  ...
  if (isFresh(meta, filePath, config.mediaCacheTtlMs, now)) {
  ...
  return downloadMutex.run(videoId, async () => {
    ...
    const recheckMeta = loadMeta(config.mediaCacheDir, videoId)
    if (isFresh(recheckMeta, filePath, config.mediaCacheTtlMs, recheckNow)) {
      persistMeta(config.mediaCacheDir, {
        ...recheckMeta,
        lastAccessedAt: recheckNow,
      })
      return filePath
    }
